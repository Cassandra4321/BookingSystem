...
export function WorkoutClassesPage() {
	...
	>([])
	const [selectedClass, setSelectedClass] = useState<WorkoutClass | null>(
		null,
	)
		...
		const fetchWorkoutClasses = async () => {
			try {
				const response = await fetch(
					'https://localhost:7193/api/WorkoutClass',
				)
				if (!response.ok) {
				...
				setWorkoutClasses((prev) =>
					prev.map((wc) =>
						wc.id === workoutClassId
							? {
									...wc,
									bookingIds: wc.bookingIds.filter(
										(id) => id !== bookingId,
									),
								}
							: wc,
			...
			<div className="container mt-5">
				...
				{selectedClass && (
					<Modal
						...
						actionLabel={
							isClassBooked(selectedClass.id) ? 'Avboka' : 'Boka'
						}
					/>
				)}
				...
				<div className="row">
					{[...workoutClasses]
						...
						.map((wc) => {
							...
							const isFull =
								wc.bookingIds.length >= wc.maxParticipants
							return (
